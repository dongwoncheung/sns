...
public class TimelineBO {
	...
	public List<CardView> generateCardViewList(Integer userId) {// 화면용을 가공을 할려고 할때 -> generateCardViewList
		List<CardView> cardViewList = new ArrayList<>(); // []
		
		// 글 목록을 가져온다.  List<PostEntity>
		List<PostEntity> postList = postBO.getPostList();
		
		// 글 목록 반복문 순회 => 글목록 들이 있는 post리스트를 분해 시켜서 cardview리스트에 넣어준다
		...
		for (PostEntity post : postList) {   // 0 1 2
			...
			// 글쓴이 정보 세팅
			UserEntity user = userBO.getUserEntityById(post.getUserId());//댓글을 쓴사람(userId)
			...
			// 좋아요 카운트
			int likeCount = likeBO.getLikeCountByPostId(post.getId());// 갯수
